...
export function AdminDashboard() {
  const { data = [], isLoading, error } = useQuery({
    queryKey: ['admin', 'games', 'recent'],
    queryFn: async (): Promise<AdminGameRow[]> => {
      ...
      return data as unknown as AdminGameRow[]
    },
  })
  ...
  return (
    <div className="py-8 px-4" style={FONT}>
      <header className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-black text-[#e2e8f0]">Admin · 比賽管理</h1>
        <Link
          to="/admin/games/new"
      ...
      {isLoading ? (
        ...
        <p className="text-[#94a3b8]">沒有比賽。</p>
      ) : (
